...
<body>
    <!-- Menyertakan modal untuk form "Tambah Data Obat" -->
                    ...
                    <div class="flex flex-col md:flex-row items-center justify-between p-4 space-y-3 md:space-y-0 md:space-x-4">
                        ...
                        <div class="w-full md:w-1/2">
                            <form id="searchForm" class="flex items-center" onsubmit="handleSearch(event)">
                                ...
                                <input type="text" id="simple-search" name="search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg w-full pl-10 p-2" placeholder="Cari obat">
                            </form>
                        </div>
                        <!-- Tombol Tambah Data -->
                        ...
                        <table class="w-full text-sm text-left text-gray-500">
                            <thead class="bg-gray-50 text-gray-700 uppercase text-xs dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-3">Fish</th>
    ...
    const fetchMedicineData = (page = 1, query = '') => {
        ...
        $.ajax({
                    ...
                    response.data.forEach(medicine => {
                        medicineTable.append(
                            `<tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3">${medicine.disease.name}</td>
                                <td class="px-4 py-3">${medicine.fish.name}</td>
                                <td class="px-4 py-3">
                                    <button onclick="openEditModal(${medicine.id})" class="bg-yellow-400 hover:bg-yellow-500 text-white font-bold py-1 px-2 rounded">Edit</button>                                                     
            ...
            error: function(xhr, status, error) {
                ...
                $('#medicineTable').html(
                    `<tr>
                        <td colspan="5" class="px-4 py-3 text-center text-red-500">Gagal memuat data</td>
                    </tr>`
                );
            }
        });
